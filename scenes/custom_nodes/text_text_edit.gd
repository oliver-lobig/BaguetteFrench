...
func _process(delta: float) -> void:
	...
	if !Engine.is_editor_hint():
		if quickchange_enabled:
			...
			if placeholder_text != last_placeholder_text:
				if BaguetteTranslationServer.ui_translations.has(placeholder_text):
					placeholder_text = BaguetteTranslationServer.translate(placeholder_text)
